  ...
  return (
    <div className="bg-white min-h-screen">
     <Header navigation1={navigation1} nasalogo={nasalogo} />
      ...
      <main className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 pt-24 px-8">
        {photoData.map((photo, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden mt-8 mb-4">
            <img
            ...
            />
            <div className="p-4">
              <p className="text-gray-600 mb-2">{photo.date}</p>
              ...
              {showModal === index && (
                <Dialog open={true} onClose={() => setShowModal(null)} className="fixed inset-0 flex items-center justify-center bg-opacity-70 backdrop-filter backdrop-blur-lg">
                  <div className="bg-white dark:bg-gray-600 rounded-lg shadow-md p-4 md:p-5">
                    <Dialog.Title as="h3" className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
                    {photo.caption}
                    </Dialog.Title>
                    <img src={`https://epic.gsfc.nasa.gov/archive/natural/${photo.date.split(' ')[0].replace(/-/g, '/')}/png/${photo.image}.png`} alt="NASA EPIC Image" className="w-64 h-auto rounded-lg mb-2 mx-auto" />
                    ...
                    <div className="space-y-4">
                      <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                       Centroid_Coordinates.Lat :{photo.centroid_coordinates.lat}
                      </p>
                    </div>
                    <div className="space-y-4">
                      <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                        Centroid_Coordinates.Lon :{photo.centroid_coordinates.lon}
                      </p>
                    </div>
                    <div className="space-y-4">
                      <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                       Dscovr_j2000_position.x : {photo. dscovr_j2000_position.x}
                      </p>
                    </div>
                    <div className="space-y-4">
                      <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                        Dscovr_j2000_position.y :{photo. dscovr_j2000_position.y}
                      </p>
                    </div>
                    <div className="space-y-4">
                      <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                       Dscovr_j2000_position.z : {photo. dscovr_j2000_position.z}
                      </p>
                    </div>
